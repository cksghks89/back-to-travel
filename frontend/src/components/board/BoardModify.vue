...
export default {
  ...
  methods: {
    ...mapActions("boardStore", ["modifyBoard", "fetchBoards"]),
    submitForm() {
      const board = {
        title: this.title,
      ...
      if (confirm("수정하시겠습니까?")) {
        this.modifyBoard({ boardId: this.getBoardDetail.boardId, board });
        this.$router.push({
          path: `/board/detail/${this.getBoardDetail.boardId}`,
        });
      }
    },
  ...
  created() {
    if (!this.getUserId) {
      this.$router.push({ path: "/noticeboard/board" });
      return;
    }
    this.title = this.getBoardDetail.title;
    this.content = this.getBoardDetail.content;
