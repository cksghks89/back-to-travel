<template>
  <div class="container">
    <h1 class="container-title">공지사항</h1>
    <div class="search">
      <input
        v-model="query"
    ...
    <table class="noticeTable">
      ...
      <tbody>
        <tr v-for="(notice, index) in getNotices" :key="index">
          ...
          <td>{{ notice.userId }}</td>
          <td>{{ notice.createTime.split(" ")[0] }}</td>
          <td>{{ notice.hit }}</td>
        </tr>
      </tbody>
    </table>
    <div class="container-bottom">
      ...
      <div class="btn-container">
        <router-link to="/notice/write">
          <button v-if="this.getUserId == 'admin'">글쓰기</button>
        </router-link>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    ...mapActions("noticeStore", ["fetchNotices"]),
    movePage(step) {
  ...
  computed: {
    ...mapGetters("noticeStore", ["getNotices", "getPageResult"]),
...
.noticeTable th:not(:nth-child(2)),
.noticeTable td:not(:nth-child(2)) {
  width: 10%;
}
.search {
  margin-top: 10px;
